<template>
    <div>
        <div class="title">
            ...
            <h1>Tabla de frases</h1>
        </div>
      <div class="container">
        <div v-if="mensaje" class="mensaje">{{ mensaje }}</div>
        <v-data-table :headers="headers" :items="frases" item-key="id" class="tabla-palabras">
          <template slot="item" slot-scope="{ item }">
            <tr>
              ...
              <td class="text-left">
                <v-btn class="editar-button" @click="EditarFrase(item[0])">
                  <v-icon>mdi-pencil</v-icon>
                </v-btn>
              </td>
              <td class="text-left">
                <v-btn class="eliminar-button" @click="eliminarFrase(item[0])">
  ...
  export default {
    ...
    methods: {
      ...
      EditarFrase(id) {
        this.$router.push(`editar-frases/editar/${id}`);
      },
      eliminarFrase(id) {
        axios
      ...
      atras() {
        this.$router.push("/Campos/ingresar-frases");
      }
    }
  };
  </script>
