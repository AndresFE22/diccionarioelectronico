<template>
    <div class="campos">
      <h1>Ingresar Frases</h1>
        <div class="form-container">
          <v-text-field v-model="frases" :rules="[[v => !!v || 'Este campo es requerido']]" :error-messages="getErrorMessages(frases)" label="Digite su frase" class="text"></v-text-field>
  ...
  export default {
    ...
    methods: {
      ...
      GuardarDatos() {
        ...
        formData.append('frases', this.frases);
        formData.append('significado', this.significado);
        formData.append('imagen', this.imagen);
        
        axios.post('https://cuentaapi.pythonanywhere.com/apie/guardarfrase', formData)
        .then(response => {
    ...
    EditarInfo() {
      this.$router.push('/Campos/editar-frases')
    },
